...
This example reproduces a ggiraph chart with two linked panels: a scatter plot
of displacement against quarter-mile time and a bar chart of miles per gallon by
car. Hovering or clicking a car in either panel highlights the matching car in
the other panel.
...
svg.querySelectorAll(".plot-element.selected").forEach((element) => {
  element.classList.remove("selected");
});

svg.querySelectorAll(".plot-element").forEach((element) => {
  if (element.getAttribute("data-group") === group) {
...
(
    ...
    + css(
        ...
        .plot-element.not-hovered:not(.selected) {
          opacity: 0.22;
        }
